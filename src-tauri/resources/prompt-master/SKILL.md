...
**Output format — the second one, when the caller asks for `FRAGE:` blocks**

*ProjectA local addendum. Not part of the upstream skill; it is what the
ProjectA preflight dialogue reads back. Keep it when updating this skill from
upstream.*

Some callers hand you a rough task description **and** state that you may ask
back instead of writing a prompt. Only those callers; when the instruction says
to output only the prompt, this section does not apply and you never emit a
`FRAGE:` line.

When it does apply, answer in **exactly one** of two ways:

1. **The task is clear enough.** Write the prompt as usual — the decorated
   output format above is overridden by whatever the caller's instruction says.
   Ask nothing.
2. **The task is too vague and guessing would send the agent the wrong way.**
   Do not guess. Output only questions — nothing before the first block and
...
Rules for the question round:

- **At most 3 questions.** A fourth is dropped unread. This is the same budget
  as the Intent Extraction rule above: max 3 clarifying questions, never more.
- One question per `FRAGE:` block, blocks separated by a blank line.
- `OPTIONEN:` is optional and belongs to the `FRAGE:` above it. Leave it out
  where a free-text answer is the only sensible one; never invent options to
  fill the line.
- **Never both.** A prompt and questions in one answer is a prompt written
  without the answers, and the caller throws the prompt away.
- Ask only what changes the prompt. "Which API, and read or write access?"
  earns its round; "what would you like me to call the variable?" does not.
- Ask in the language the task is written in — a person is reading these, not a
  model.
- No preamble, no numbering, no closing remark, no explanation of why you are
  asking.

The answers come back in a second call, under `=== RUECKFRAGEN UND ANTWORTEN
===`, together with the original task. That round writes the prompt and asks
nothing further.
