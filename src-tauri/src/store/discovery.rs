...
pub(super) async fn apply_migration(tx: &mut Transaction<'_, Sqlite>) -> Result<(), String> {
    ...
    sqlx::query(
        "CREATE INDEX continuous_discovery_day ON continuous_discovery(project_id,utc_day)",
    )
    .execute(&mut **tx)
    .await
    .map_err(db)?;
...
fn db(error: sqlx::Error) -> String {
    format!("discovery admission: {error}")
}

impl Store {
    /// Trusted coordinator only; no agent-facing admission endpoint. Reserve
    /// before any scan starts. A lost response consumes a slot: read context to
    /// reconcile, never replay a permit. Model work additionally needs its root
    /// token reservation and verified provider route before launch.
    #[allow(dead_code)] // Coordinator dispatch is still gated.
    pub async fn reserve_discovery_scan(
        &self,
        goal: &str,
        operation_key: &str,
    ) -> Result<DiscoveryPermit, String> {
        self.reserve_discovery_at(goal, operation_key, now_unix_secs())
            .await
    }
...
mod tests {
    ...
    async fn fixture() -> (TempDir, Store, String, String) {
        ...
        let project = store
            .create_project("discovery", &dir.path().to_string_lossy())
            .await
            .unwrap();
        let goal = store
    ...
    async fn discovery_race_restart_and_replan_share_daily_capacity() {
        let (dir, store, project, goal) = fixture().await;
        let child = store
            .create_continuous_goal(&project, "replan", None, Some(goal.clone()), true)
            .await
            .unwrap();
        let now = now_unix_secs();
        let (a, b, c) = tokio::join!(
            store.reserve_discovery_at(&goal, "a", now),
            store.reserve_discovery_at(&child.id, "b", now),
            store.reserve_discovery_at(&goal, "c", now)
        );
        assert_eq!(
        ...
        let accepted = a.or(b).or(c).unwrap();
        let snapshot = store.continuous_context(&project, 0).await.unwrap();
        assert_eq!(snapshot.discovery["reservedToday"], 2);
        assert_eq!(snapshot.discovery["executionState"], "unavailable");
        assert_eq!(
            snapshot
                .events
                .iter()
                .filter(|e| e.kind == "discovery_reserved")
                .count(),
            2
        );
        ...
        assert!(reopened
            .reserve_discovery_at(&goal, &accepted.operation_key, now)
            .await
            .unwrap_err()
            .contains("already reserved"));
        assert!(reopened
            .reserve_discovery_at(&goal, "new", now)
            .await
            .unwrap_err()
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn discovery_day_boundary_rollback_and_duplicate_are_conservative() {
        let (_dir, store, project, goal) = fixture().await;
        let day = now_unix_secs().div_euclid(86_400);
        let before = day * 86_400 + 86_399;
        sqlx::query("UPDATE continuous_goals SET deadline_at=?")
            .bind(before + 500)
            .execute(&store.pool)
            .await
            .unwrap();
        store
            .reserve_discovery_at(&goal, "a", before)
            .await
            .unwrap();
        store
            .reserve_discovery_at(&goal, "b", before)
            .await
            .unwrap();
        assert!(store
            .reserve_discovery_at(&goal, "c", before - 1)
            .await
            .unwrap_err()
        ...
        store
            ...
            .unwrap();
        assert!(store
            .reserve_discovery_at(&goal, "a", before + 1)
            .await
            .unwrap_err()
            .contains("already reserved"));
        sqlx::query("CREATE TRIGGER fail_discovery_journal BEFORE INSERT ON continuous_events WHEN NEW.kind='discovery_reserved' BEGIN SELECT RAISE(ABORT,'injected journal failure'); END").execute(&store.pool).await.unwrap();
        assert!(store
            .reserve_discovery_at(&goal, "rollback", before + 1)
            .await
            .is_err());
        let (count,): (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM continuous_discovery WHERE project_id=?")
                .bind(&project)
                .fetch_one(&store.pool)
                .await
                .unwrap();
        assert_eq!(count, 3);
        sqlx::query("DROP TRIGGER fail_discovery_journal")
            .execute(&store.pool)
            .await
            .unwrap();
        store
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn discovery_disabled_expired_and_invalid_requests_do_not_consume_slots() {
        let (_dir, store, project, goal) = fixture().await;
        let now = now_unix_secs();
        for key in ["", " spaced", "bad\nkey"] {
            assert!(store.reserve_discovery_at(&goal, key, now).await.is_err());
        }
        for state in ["paused", "draining"] {
            sqlx::query("UPDATE continuous_projects SET status=?")
                .bind(state)
                .execute(&store.pool)
                .await
                .unwrap();
            assert!(store.reserve_discovery_at(&goal, "a", now).await.is_err());
        }
        ...
        policy.continuous.discovery_runs_per_day = 0;
        sqlx::query("UPDATE continuous_root_policies SET policy_json=?")
            ...
            .unwrap();
        assert!(store
            .reserve_discovery_at(&goal, "a", now)
            .await
            .unwrap_err()
            .contains("capacity exhausted"));
        sqlx::query("UPDATE continuous_goals SET deadline_at=?")
    ...
    async fn discovery_scan_reports_unreadable_dependencies_and_truncation() {
        ...
        // Two worker slots (policy default), filled in creation order; every
        // later ready task is a capacity skip (delta review KD2/GD4).
        let ids: Vec<&str> = scan.dispatched.iter().map(|d| d.task_id.as_str()).collect();
        assert_eq!(ids, ["t001", "t002"]);
        assert!(scan.skipped[1..]
            .iter()
            .all(|skip| skip.reason == "worker_capacity_exhausted"));
        assert!(scan
            .skipped
            .iter()
    ...
    async fn malformed_dispatch_journal_row_does_not_break_context() {
        ...
        // A scanned permit keeps its recorded state next to the malformed row
        // (delta review KD1/GD3).
        let scan = store
            .scan_discovery_at(&goal, "scanned", now_unix_secs())
            .await
            .unwrap();
        sqlx::query("INSERT INTO continuous_events(project_id,kind,detail,created_at) VALUES(?,'discovery_dispatched','not json',1)")
            .bind(&project)
            ...
            .unwrap();
        let snapshot = store.continuous_context(&project, 0).await.unwrap();
        let recent = snapshot.discovery["recent"].as_array().unwrap();
        let entry = |id: &str| {
            recent
                .iter()
                .find(|permit| permit["id"] == id)
                .unwrap()
                .clone()
        };
        assert_eq!(entry(&scan.permit.id)["dispatchState"], "recorded");
        assert_eq!(snapshot.discovery["latestDispatch"]["state"], "unreadable");
    }
