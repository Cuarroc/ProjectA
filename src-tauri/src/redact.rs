...
//! unless the word ends in a hex character itself; see that function for the
//! gaps.
...
/// and `diagnosis.rs` exports (whose test fixtures at `diagnosis.rs:430-445`
/// plant exactly this shape as the API/verdict token and hook secret the
/// export must strip - there the shape *is* the secret). However
...
///
/// **Known false positives:** the ones [`is_token_shaped`] lists now also
/// fire when glued to word characters - a `md5_<hash>` filename, a glued
/// `uuid4().hex`, and the trace id inside a W3C `traceparent` header
/// (`00-<32 hex>-<16 hex>-01` is one segment and becomes
/// `00-[redacted]-<16 hex>-01`). Accepted for the same reason: a leaked token
/// is worse than a trace id that has to be looked up elsewhere.
fn mask_token_runs(segment: &str) -> String {
    ...
    while start < bytes.len() {
        ...
        // A run boundary is where hex-ness flips. Hex bytes are ASCII, and an
        // ASCII byte is never part of a multi-byte UTF-8 sequence, so every
        // boundary is a char boundary and the slice cannot panic.
...
mod tests {
    ...
    fn hex_runs_of_other_lengths_inside_a_segment_are_not_masked() {
        ...
        // Accepted false positive (see mask_token_runs): the trace id inside a
        // W3C traceparent header is exactly the token shape.
        assert_eq!(
            redact("traceparent: 00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01"),
            "traceparent: 00-[redacted]-00f067aa0ba902b7-01"
        );
    ...
    fn a_token_split_at_every_offset_is_still_masked() {
        ...
        for text in [
            ...
            token.to_string(),
            // Non-ASCII around the token: cuts must stay on char boundaries.
            format!("präfix tok_{token} ✅\n"),
            format!("λ=verdict-{token}_x"),
        ] {
            let whole = redact(&text);
