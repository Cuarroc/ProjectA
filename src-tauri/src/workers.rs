...
mod tests {
    ...
    async fn a_coordinator_starts_outside_every_checkout_without_credentials() {
        ...
        let orchestrator = create_orchestrator(&fx.store, &agents, &fx.project_id, None)
            .await
            .unwrap();
        // Queen creation is retired, so the queen is a historical row that
        // only ever comes back through a respawn.
        let queen = WorkerRow {
            id: "wk-queen".to_string(),
            project_id: fx.project_id.clone(),
            task: "Queen: Backend".to_string(),
        ...
        fx.store.insert_worker(&queen).await.unwrap();
