# Review request PR #9 (W5-22): conflict forecast and seam-lane guard — final candidate
...
public GitHub repo). One implementation package = one agent = one git
worktree. Four "seam" files may be edited by only one package at a time
(serial lanes): `src-tauri/src/store.rs` together with `src-tauri/src/store/**`
(lane st), `src-tauri/src/api.rs` (api), `src-tauri/src/main.rs` (mn),
`src-tauri/src/bin/pa.rs` (pa). Development runs live in SQLite
(`store/development_runs.rs`, states intent / launched / reconciling / ...);
a task claim (`running`) is held separately from the run row. Scope locks
(`continuous_scope_locks`) serialize only IDENTICAL paths at task admission.

## Package requirement (plan wording, translated)

W5-22 "conflict forecast + lane guard": before a package starts, forecast
conflicts from the files it will touch; a second start on an occupied seam
lane is refused while the first package is open; overlapping non-seam paths
only warn. The forecast must be validated by back-calculating the real merge
order of wave 2 (2026-09-23). The seam files themselves must not be edited
by this package.

## Review history (already addressed — verify, don't just re-report)

Two earlier reviewers (kimi-k3, glm-5.2) reviewed candidate 27cdb27, then the
delta ad0e8c6. Dispositions in the diff:

- `let _ = insert_message` replaced by a logged warning (eprintln).
- Back-calculation test made discriminating: a synthetic test proves the
  conflict edges (not priorities alone) produce the serialization.
- Test asserting that a held task claim keeps its lane was added.
- Mojibake finding rejected as prompt-encoding artifact (source is UTF-8).
- "Orphan intent run holds the lane" answered: the lane is held by the claim;
  orphan intent runs are pre-existing behavior.
- Known and accepted limits: the check is advisory, not transactional
  (TOCTOU between two concurrent launchers; only the trusted launch lane calls
  it today, no production scheduler exists yet); the legacy `task_queue` has
  no path metadata and is out of scope. Follow-ups N-01 (`is_err()` might
  match the holder) and N-02 (unify format args) are registered.
- Commit 57017f4 exempts the lane_guard.rs test fixture (real merged-PR file
  lists) from the doc-literal scan in scripts/ci/lane-plan.sh.

Check these fixes are actually correct and hunt for NEW bugs anywhere:
path normalization holes (`..`, absolute paths, `src-tauri\src\STORE.RS`,
symlinks, drive prefixes), seam classification, lane-holding semantics (which
run/claim states count as open, self-conflict with the candidate's own run),
the async store adapter, the wiring position in `launch_worker`, the Kahn
ordering (determinism, cycles, tie-breaks, unknown deps), and test quality
(do the tests fail without the implementation?).

## Diff (git diff origin/main...HEAD, complete final candidate 57017f4)

```diff
diff --git a/scripts/ci/lane-plan.sh b/scripts/ci/lane-plan.sh
index 2f7a1e0..b7fae2f 100644
--- a/scripts/ci/lane-plan.sh
+++ b/scripts/ci/lane-plan.sh
...
+    let lane_warnings = super::lane_guard::refuse_lane_conflicts(store, &run_record).await?;
     let launch = store
         ...
         .await?;
@@ -169,6 +173,28 @@ pub async fn launch_worker(
...
diff --git a/src-tauri/src/workers/lane_guard.rs b/src-tauri/src/workers/lane_guard.rs
new file mode 100644
index 0000000..8101475
--- /dev/null
+++ b/src-tauri/src/workers/lane_guard.rs
@@ -0,0 +1,903 @@
