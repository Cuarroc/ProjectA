...
## Diff des PRs (origin/claude/w1-25-sqlite-load..HEAD, Stand 1d6eef6)

Hinweis: Ein erstes Review (deepseek-v4-pro) hat bereits stattgefunden; daraus übernommen: DELETE+SELECT in einer Transaktion, Body-Prüfung im API-Test, Abdeckungshinweis am Wächtertest. Du reviewst den Stand danach unabhängig.

```diff
diff --git a/src-tauri/src/api.rs b/src-tauri/src/api.rs
index 019ba48..88e1547 100644
...
@@ -6526,6 +6542,31 @@ pub(crate) mod tests {
         }
     }
...
index c31c4f9..5fd6c7b 100644
--- a/src-tauri/src/store.rs
+++ b/src-tauri/src/store.rs
@@ -2714,18 +2714,54 @@ impl Store {
         Ok(())
     }
...
+    /// later. The transaction is committed on every path, so a failing commit
+    /// is reported rather than swallowed by a drop.
...
+                sqlx::query_as("SELECT status FROM task_queue WHERE id = ?1")
+                    .bind(id)
+                    .fetch_optional(&mut *tx)
+                    .await
...
@@ -6858,4 +6894,50 @@ pub(crate) mod tests {
