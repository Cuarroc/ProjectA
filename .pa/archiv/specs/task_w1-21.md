# W1-21: xterm-Search-Addon

Status: historisch

Abgeschlossen: gemergt mit PR #74 (Zeile in `docs/ERLEDIGT.md`); diese Spec ist nur noch Beleg.
...
- `TerminalView.tsx`, `package.json` (`@xterm/addon-search`, Version zum xterm-5.5-Paar passend)

## Abnahme

- Suche im Scrollback per Tastenkürzel, Screenshot
...
- Diff > 300 Zeilen oder Nahtstelle = zwei unabhaengige Reviews mit
  Disposition in `.pa/review_w1-21_*.md`; sonst ein Reviewer ungleich Autor.
- Ein PR je Paket gegen `main`, CI gruen (gates linux/windows, red-first).

## Abschluss

Report `.pa/report_w1-21.md` (Belege, Gates, Review-Disposition), diese Datei
