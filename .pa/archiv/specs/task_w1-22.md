...
Groesse: S. Lane: seriell `main.rs` (nach W1-15). Weg: Codex.

## Ziel und Vertrag (aus PLAN.md)

- Lane **main.rs** (nach W1-15), `logging.rs`, `docs/plugin-matrix.md`

## Abnahme

- Redaction-Canaries bleiben leer, Diagnostics zeigt weiter den Logpfad
...
- Diff > 300 Zeilen oder Nahtstelle = zwei unabhaengige Reviews mit
  Disposition in `.pa/review_w1-22_*.md`; sonst ein Reviewer ungleich Autor.
- Ein PR je Paket gegen `main`, CI gruen (gates linux/windows, red-first).

## Abschluss

Report `.pa/report_w1-22.md` (Belege, Gates, Review-Disposition), diese Datei
