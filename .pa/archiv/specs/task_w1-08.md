...
Groesse: S. Lane: parallel (keine Nahtstelle). Weg: Codex.

## Ziel und Vertrag (aus PLAN.md)

- `budget.rs`, `quota.rs` (`resets_at` clampen), `learnings.rs` (Marker-Verbot vs. `one_line`), `providers.rs`/`omniroute.rs` (Vault-Schlüssel nur an verifizierten Router)

## Abnahme

- je Befund roter Test oder Beleg-Report, dass der Befund nicht mehr zutrifft
...
- Diff > 300 Zeilen oder Nahtstelle = zwei unabhaengige Reviews mit
  Disposition in `.pa/review_w1-08_*.md`; sonst ein Reviewer ungleich Autor.
- Ein PR je Paket gegen `main`, CI gruen (gates linux/windows, red-first).

## Abschluss

Report `.pa/report_w1-08.md` (Belege, Gates, Review-Disposition), diese Datei
