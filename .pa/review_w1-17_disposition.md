...
| K2 | kimi K2, glm K5 | low | No test pins escaping of PLAN.md text in `milestoneTable` / `milestoneProgress`, nor the rendered state labels and `done/total` | **Accepted.** New test `W1-17 milestone views escape PLAN.md text and label every state` in `hq-a11y.test.mjs` (hostile title/lane/stand, all four state labels, fraction on Map and Now). |
